...
function onTick()
    ...
    KM = scale(transform(Ppred, H),1 / (dot(H,transform(Ppred, H)) + R))
	X = addVec(pred, scale(KM, (x - dot(H,pred))))
    ...
    KM = scale(transform(Ppred, H),1 / (dot(H,transform(Ppred, H)) + R))
	Y = addVec(pred, scale(KM, (y - dot(H,pred))))
    ...
    KM = scale(transform(Ppred, H),1 / (dot(H,transform(Ppred, H)) + R))
	Z = addVec(pred, scale(KM, (z - dot(H,pred))))
	P_X = sub(Ppred, scaleMat(Ppred, dot(KM,H)))

    -- output.setNumber(1, X[1])
    -- output.setNumber(2, X[2])

    -- output.setNumber(3, Y[1])
    -- output.setNumber(4, Y[2])

    -- output.setNumber(5, Z[1])
    -- output.setNumber(6, Z[2])
